...
const generateCollectionScript = data => {
	...
	const modelDefinitions = [
		...parseDefinitions(data.modelDefinitions),
		...internalDefinitions.filter(def => def.isTopLevel),
	];
...
const getDefinitionStatements = ({
	...
	if (jsonSchema.type === 'message') {
		return getMessageStatement({
			jsonSchema: { ...jsonSchema, options: jsonSchema.fieldOptions },
			spacePrefix,
			protoVersion,
			internalDefinitions,
			modelDefinitions,
...
const getMessageStatement = ({
	...
	const options = jsonSchema.options
		? jsonSchema.options.map(option => getOptionStatement(option, spacePrefix + ROW_PREFIX))
		: [];
	...
	const messageDefinitions = [...innerDefinitions, ...extractedDefinitions]
		.map(definition =>
			getDefinitionStatements({
				jsonSchema: definition,
				spacePrefix: spacePrefix + ROW_PREFIX,
...
const getOneOfStatement = (oneOfMeta, fields, spacePrefix = '') => {
	...
	const statement = [`${spacePrefix}oneof ${oneOfName} {`, ...fields, '}'].join(`\n${spacePrefix}`);
...
const getEnumStatement = ({ jsonSchema, spacePrefix = '' }) => {
	...
	return [`${spacePrefix}enum ${jsonSchema.title} {`, ...options, ...constants, `}`].join(`\n${spacePrefix}`);
};
...
const getEnumOptions = options => {
	...
	return options
		...
		.map(option => getOptionStatement(option, ROW_PREFIX));
};
...
const getReservedStatements = (data, spacePrefix) => {
	...
	const reservedFieldNames = !_.isEmpty(data.reservedFieldNames)
		...
		: ``;
	const reservedFieldNumbers = !_.isEmpty(data.reservedFieldNumbers)
		? `${spacePrefix}reserved ${data.reservedFieldNumbers};`
		: ``;
	return {
		reservedFieldNames,
		reservedFieldNumbers,
...
const getFieldsStatement = ({
	...
	const { collectionProperties, oneOfProperties } = fixFieldNumbers({
		fields: { ...jsonSchema.properties },
		oneOfFields,
		reservedNumbers: jsonSchema.reservedFieldNumbers,
		oneOfIndex,
	});
...
const convertFieldsToStatements = ({
	fields,
	spacePrefix,
	protoVersion,
	internalDefinitions,
	...
	return Object.keys(fields).map(fieldName => {
		...
		const { fieldType, fieldOptions, hasReferenceError } = getFieldInfo({
			field,
			isReference,
			isExternalRef,
			internalDefinitions,
