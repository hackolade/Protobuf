...
const extractDefinitionsFromProperties = (properties = []) => {
	const { extractedDefinitions, modifiedProperties } = Object.entries(properties).reduce(
		({ extractedDefinitions, modifiedProperties }, [key, value]) => {
 ...
 * @returns {string[]}
 */
const getMessageUsedModelDefinitionNames = ({ jsonSchema, modelDefinitions, internalDefinitions }) => {
	...
	const usedModelDefinitionNamesInInternalDefinitions = internalDefinitions.flatMap(definition => {
