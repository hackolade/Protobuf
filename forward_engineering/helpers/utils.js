...
const formatComment = comment => {
	return !comment
		...
		: comment
				.replaceAll('\n', '')
				.split(' ')
				.reduce((descriptionComment, word, i) => {
					return descriptionComment + ' ' + word + (i % MAX_CHAR_LENGTH === MAX_CHAR_LENGTH - 1 ? '\n*' : '');
				}, '/*') + '*/\n';
};
...
const wrapInCommentBlock = ({ statement, spacePrefix }) => {
	...
	const commentBody = statement.split('\n\n').map(getCommentBodyLine).join(`\n${spacePrefix}*\n`);
