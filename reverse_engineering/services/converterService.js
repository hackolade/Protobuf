...
const convertParsedFileDataToCollections = (parsedData, fileName) => {
	...
	const modelDefinitionsNames = [
		...
		..._.get(parsedData, 'messages', [])
			.filter(parsedMessage => parsedMessage.name !== rootMessageName)
	...
	const options = _.get(parsedData, 'options', []).map(option => ({
	...
	const formattedModelDefinitions = modelDefinitions
		...
		.reduce((definitions, def) => ({ ...definitions, [def.name]: _.omit(def, ['name']) }), {});
...
const getJsonSchema = (message, modelDefinitionsNames, modelDefinitions, hackoladeGeneratedDefsNames) => {
	...
	const { reservedFieldNumbers, reservedFieldNames } = message.body
		...
		.reduce(
			(reservedValues, reservedString) => {
				const formattedString = reservedString.values.join(', ');
				if (formattedString.match(/^\d+(?:(?:,\s*\d+)|(?:,\s+\d+\s+to\s+\d+)|(?:,\s+\d+\s+to\s+max))*$/gm)) {
					return {
						...reservedValues,
						reservedFieldNumbers: [...reservedValues.reservedFieldNumbers, formattedString],
					};
				} else {
					return {
						...reservedValues,
						reservedFieldNames: [
							...reservedValues.reservedFieldNames,
							reservedString.values.map(str => `'${str}'`).join(', '),
						],
					};
				}
			},
...
const messageFieldConverter = ({
	...
	const entitiesDefinitionsTypes = message.body
		.filter(field => field)
		.filter(field => [MESSAGE_TYPE, ENUM_TYPE].includes(field.elementType));
	const formattedEntitiesDefinitionTypes = entitiesDefinitionsTypes.reduce(
		(entitiesMap, entity) => ({ ...entitiesMap, [entity.name]: entity }),
		{},
	);
	const properties = message.body
		.filter(field => field)
		...
		.reduce((fields, field) => {
			const messageWithUpperLevelDefinitions = {
				...field,
				body: [..._.get(field, 'body', []), ...entitiesDefinitionsTypes],
			};
			return {
				...fields,
				[field.name]: getConverter(field.elementType)({
